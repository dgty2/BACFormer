...
def main():
    ...
    parser.add_argument("--num_classes", default=1, type=int, help="左心房=1类（二分类）")
    parser.add_argument("--img_size", default=224, type=int, help="输入图像尺寸")
    parser.add_argument("--batch_size", default=1, type=int, help="批次大小（小显存电脑用1）")
    ...
    # 加载数据集
    train_dataset = SynapseDataset(args.root_path, args.list_dir, split="train", img_size=args.img_size)
    val_dataset = SynapseDataset(args.root_path, args.list_dir, split="test_vol", img_size=args.img_size)  # 原项目用test_vol做验证
