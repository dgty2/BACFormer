...
class LAMeasureWindow(QMainWindow):
    ...
    def __init__(self, weight_path="weights/best_model.pth"):
        ...
        # 射血分数相关变量
        self.lavmax_value = None  # 舒张末期容积
        self.lavmin_value = None  # 收缩末期容积
    ...
    def _init_ui(self):
        ...
        self.calc_lavmin_btn = QPushButton("计算 LAVmin")
        self.calc_lavmin_btn.clicked.connect(self._calculate_lavmin)
        self.calc_lavmin_btn.setEnabled(False)  # 初始禁用
        calc_layout.addWidget(self.calc_lavmin_btn)

        self.calc_ef_btn = QPushButton("计算射血分数 EF")
        self.calc_ef_btn.clicked.connect(self._calculate_ef)
        self.calc_ef_btn.setEnabled(True)  # 一直启用，因为跨文件
        calc_layout.addWidget(self.calc_ef_btn)
    ...
    def _open_nii_file(self):
        ...
        try:
            ...
            # 清空当前文件的临时结果，保留跨文件的LAVmax/LAVmin
            self._clear_results()
    ...
    def _auto_segment(self):
        ...
        try:
            ...
            self.calc_lavmin_btn.setEnabled(True)
            # 恢复按钮状态
    ...
    def _calculate_lavmax(self):
        ...
        基于当前掩码计算左心房面积、长轴长度、LAVmax容积和Dice系数
        """
        # 检查是否有掩码
    ...
    @pyqtSlot()
    def _calculate_lavmin(self):
        """
        槽函数：计算LAVmin和相关指标

        基于当前掩码计算左心房收缩末期容积LAVmin
        """
        # 检查是否有掩码
        if self.mask is None:
            QMessageBox.warning(self, "提示", "请先完成分割/加载掩码")
        ...
        try:
            ...
            # 计算几何参数
            area, long_axis = calculate_la_geometry(mask_slice, self.pixdim)
            # 计算LAVmin（使用与LAVmax相同的单平面计算方法）
            lavmin = calculate_lavmax_single_plane(area, long_axis)
            ...
            self.area_edit.setText(f"{area:.2f}")
            self.long_axis_edit.setText(f"{long_axis:.2f}")
            self.lavmin_edit.setText(f"{lavmin:.2f}")
            self.dice_edit.setText(dice_str)
            ...
            QMessageBox.information(self, "计算完成",
                                    f"LAVmin：{lavmin:.2f} mL<br>Dice：{dice_str}")
        except Exception as e:
            QMessageBox.critical(self, "错误", f"计算失败：{str(e)}")
    ...
    def _calculate_ef(self):
        ...
        基于LAVmax和LAVmin计算左心房射血分数
        EF = (LAVmax - LAVmin) / LAVmax * 100%
        """
        # 检查是否已计算两个容积值
        if self.lavmax_value is None:
            QMessageBox.warning(self, "提示", "请先计算 LAVmax（ES文件的容积）")
            return
        if self.lavmin_value is None:
            QMessageBox.warning(self, "提示", "请先计算 LAVmin（ED文件的容积）")
            return

        try:
            # 检查LAVmax是否为0，防止除零错误
            if self.lavmax_value <= 0:
                QMessageBox.warning(self, "提示", "LAVmax值无效，无法计算射血分数")
                return

            # 计算射血分数
            ef = (self.lavmax_value - self.lavmin_value) / self.lavmax_value * 100

            # 更新显示
            self.ef_edit.setText(f"{ef:.2f}")

            # 提示用户
            if ef < 0:
                QMessageBox.warning(self, "计算完成",
                                    f"射血分数：{ef:.2f}%<br>注意：检测到LAVmin大于LAVmax，可能您计算的顺序反了，请确认！")
            else:
                QMessageBox.information(self, "计算完成",
                                        f"射血分数：{ef:.2f}%")
        except Exception as e:
    ...
    def _export_report(self):
        ...
        try:
            ...
            # 读取新增的LAVmin和EF值
            lavmin = float(self.lavmin_edit.text()) if self.lavmin_edit.text() else None
            ef = float(self.ef_edit.text()) if self.ef_edit.text() else None
    ...
    def _clear_results(self):
        """
        清空当前文件的临时测量结果，保留跨文件的LAVmax/LAVmin结果
        """
        # 只清空当前文件的临时结果
        ...
        self.calc_lavmin_btn.setEnabled(False)

        # 不要清空跨文件的LAVmax、LAVmin、EF的结果，也不要重置对应的变量
        # 因为这些是两个不同文件的计算结果，需要保留下来计算EF

        # 导出按钮只要有结果就启用
        self.export_btn.setEnabled(self.lavmax_value is not None or self.lavmin_value is not None)
